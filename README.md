...
최근에는 MVP 후속 개선으로 스캔/프리뷰 성능, 그룹 편집 UX 연결, 지도 렌더링 최적화, 메인 프로세스 조립 정리까지 1차 반영됐습니다.
...
- 스캔/프리뷰 준비 단계 제한 병렬 처리
- 기존 출력 해시 계산 제한 병렬 처리
- 대표 사진 프리뷰 생성 제한 병렬 처리
- EXIF 기본 파싱 + 필요 시 XMP/IPTC 보조 파싱
- 지도 브라우즈 화면에서 그룹 상세 편집 연결
  - 제목 수정
  - 동행인 수정
  - 메모 수정
  - 대표 사진 변경
...
- 설치 파일 재배포를 통한 수동 업데이트는 가능하지만, 자동 업데이트는 아직 구현하지 않았습니다.
...
`dist` 스크립트는 내부적으로 `pnpm build && electron-builder` 를 실행합니다.

## 배포 / 업데이트 방식

현재 배포 방식은 수동 배포 기준입니다.

- `corepack pnpm build`
  - Electron/Vite 번들만 생성합니다.
- `corepack pnpm dist`
  - 번들 생성 후 `electron-builder` 로 Windows 설치 파일을 만듭니다.
- 새 버전 배포 시
  - 새 설치 파일을 다시 전달합니다.
  - 사용자는 새 설치 파일을 직접 실행해 덮어설치합니다.
...
10. 필요 시 그룹 안 사진 일부를 다른 그룹으로 이동
...
- 스캔/프리뷰 성능 개선
  - 준비 단계 제한 병렬 처리
  - 기존 출력 해시 계산 최적화
  - 프리뷰 이미지 생성 과도한 병렬화 완화
- EXIF 파싱 최적화
  - 일반 사진은 빠른 기본 파싱 우선
  - 필요한 경우에만 XMP/IPTC 보조 파싱
- 그룹 편집 UX 연결
  - 지도 화면에서 그룹 상세 편집 탭 제공
  - 저장/이동 후 인덱스 리로드 및 성공 피드백
- 지도 렌더링 최적화
  - 마커 diff 업데이트
  - 전체 마커 재생성 감소
- 구조 정리
  - duplicate canonical 계산 공통화
  - Electron main 팩토리 분리
  - renderer의 제목 포맷 경계 정리
